...
class GraphLoader():
    ...
    def __init__(self, config, user2id=None, item2id=None):
        ...
        train_behavior_file = path + config.graph_filename #重新构建, 每行由<head, rel, tail> 构成;
            ...
            for l in f.readlines():
                if len(l) > 0:
                    ...
                    if attribute not in self.attribute2id:
                        self.attribute2id[attribute] = len(self.attribute2id) + len(item2id)
        ...
        # 构建behavior-level的数据
        rel_behavior = "behavior_rel"
        if rel_behavior not in self.rel2id:
            self.rel2id[rel_behavior] = len(self.rel2id)
        
        rel_behavior_index = self.rel2id[rel_behavior]

        with open(train_behavior_file) as f:
            for l in f.readlines():
                if len(l) > 0:
                    l = l.strip('\n').split(' ')
                    ...
                    self.all_head_list.extend([uid] * len(items))
                    self.all_rel_list.extend([rel_behavior_index] * len(items))
                    self.all_tail_list.extend(items)
        ...
        self.origin_m_item = len(item2id) + 1
        self.origin_m_user = len(item2id) + 1

        self.m_item = len(item2id) + len(self.attribute2id) + 1
        self.n_user = len(item2id) + len(self.attribute2id) + 1
