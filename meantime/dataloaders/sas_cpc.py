    ...
    def __getitem__(self, index):
        ...
        if self.marank_mode:
            ...
            for index in range(len(labels)):
                label_single = labels[index: min(index+target_len, len(labels))]
                if type(label_single) == int:
                    label_single = [label_single]
                label_single += [0]* (target_len - len(label_single))
                label_fur.append(label_single)
            ...
            for _ in labels:
                ...
                for _ in range(neg_len):
                    negative_item.append(self.rng.choice(neg_samples))
                negative_labels.append(negative_item)

            tokens = [0] * padding_len + tokens
            labels = [[0 for _ in range(target_len)] for _ in range(padding_len)] + label_fur
            negative_labels = [[0 for _ in range(neg_len)] for _ in range(padding_len)] + negative_labels

        # pdb.set_trace()
        # 数据增强
        d = {
            'tokens': torch.LongTensor(tokens), #(sl)
            'labels': torch.LongTensor(labels), #(sl, target_len)
            'negative_labels': torch.LongTensor(negative_labels,) #(sl, neg_len)
        }
