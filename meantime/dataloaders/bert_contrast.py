...
class BertConstrastTrainDataset(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args.argument_type == "cutoff_subseq":
            ...
            d['tokens_pair'] = torch.LongTensor(sub_tokens)
            # pdb.set_trace()
        ...
        if self.args.argument_type == "shuffle_subseq":
            ...
            d['tokens_pair'] = torch.LongTensor(shuffle_subseq)
            # pdb.set_trace()
        #构建mask输入数据;
        tokens = []
        labels = []
        ...
        #为mask items创建字典;
        if self.args.argument_type == "mask_seq":
            d['tokens_pair'] = torch.LongTensor(tokens)
