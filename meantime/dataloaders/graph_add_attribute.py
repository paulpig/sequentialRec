...
class GraphLoader():
    ...
    def __init__(self, config, user2id=None, item2id=None):
        ...
        train_attribute_file = path + config.train_attribute_file

        print("Loading datafile is: ", train_file, train_attribute_file)
        # train_file = path
        ...
        attribute2id = {}
        max_item_id = max([item[1] for item in item2id.items()]) + 1
        with open(train_file) as f:
            for l in f.readlines():
                if len(l) > 0:
                    l = l.strip('\n').split(' ')
                    ...
                    # pdb.set_trace()
                    item_id = item2id[l[0]]
                    if l[1] not in attribute2id:
                        attribute2id[l[1]] = max_item_id + len(attribute2id)
                    
                    attribute_id = attribute2id[l[1]]
                    
                    #convert string to int
                    # uid = user2id[l[0]] #暂时不考虑user对模型的影响, 只考虑items共现的影响;
                    # uid = int(l[0])
                    # trainUniqueUsers.append(uid)
                    trainUser.append(item_id)
                    # trainUser.extend([uid] * len(items))
        ...
        self.m_item = max([item[1] for item in attribute2id.items()]) + 1
        self.n_user = max([item[1] for item in attribute2id.items()]) + 1
