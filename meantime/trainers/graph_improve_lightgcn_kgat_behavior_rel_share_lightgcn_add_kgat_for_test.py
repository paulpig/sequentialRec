...
class GraphTrainer(AbstractTrainer):
    ...
    def code(cls):
        ...
        return 'graph_sasrec_improve_lightgcn_kgat_behavoir_rel_items_shared_lightgcn_add_kgat_for_statistically'
    ...
    def train(self):
        ...
        self.user_hidden_rep_cate_ori, self.item_hidden_rep_cate_ori = self.graph_model_kgat.getUserItemEmbOri()

        #setting representations to sequential models; 由于user embedding不参与模型, 两个输出的均是item表征;
        self.model.setUserItemRepFromGraph(self.user_hidden_rep, self.item_hidden_rep, self.user_hidden_rep_cate, self.item_hidden_rep_cate, self.user_hidden_rep_ori, self.item_hidden_rep_ori, self.user_hidden_rep_cate_ori, self.item_hidden_rep_cate_ori) #每次加载相同的hidden representatin, 不合理;
        ...
        for epoch in range(self.epoch_start, self.num_epochs):
            ...
            if stop_training:
                ...
                weight_path = best_model_logger.filepath() #检索最有模型路径, 修改路劲是最优路径....
                if self.use_parallel:
        ...
        self.logger_service.complete({
            'state_dict': (self._create_state_dict(epoch, accum_iter, is_test=True)),
    ...
    def validate_for_statistic(self, mode):
        dummy_epoch, dummy_accum_iter = 0, 0
        # pdb.set_trace()
        result = []
        for index in range(100):
            val_log_data = self.validate(dummy_epoch, dummy_accum_iter, mode)

            ndcg_10 = val_log_data[self.best_metric]
            result.append(ndcg_10)
    ...
    def validate(self, epoch, accum_iter, mode, doLog=True, **kwargs):
        ...
        if mode == 'test':
            self.model.saveTensorAsWeight() #创建tmp
            # pdb.set_trace()
            self._restore_best_state_model(is_best=True)
            self.model.reloadWeightForTest()
        import random
        # pdb.set_trace()
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                if self.pilot and batch_idx >= self.pilot_batch_cnt:
                    ...
                    break
                if mode == 'test':
                    val = random.random() #控制只有80%数据可以pass;
                    if val > 0.1:
                        # pdb.set_trace()
                        continue
                batch = {k:v.to(self.device) for k, v in batch.items()}
            ...
            if doLog:
                if mode == 'val':
                    ...
                    log_data_v2.update(average_meter_set.averages())
                    log_data_v2.update(kwargs)
                    
                    self.logger_service.val_loggers[-1].log(**log_data_v2) # Best模型存放的参数;
                elif mode == 'test':
                    # log_data_v2 = {
                    #     'state_dict': (self._create_state_dict(epoch, accum_iter, is_test=True)),
                    #     'epoch': epoch,
    ...
    def _create_state_dict(self, epoch, accum_iter, is_test=False):
        # pdb.set_trace()
        if is_test:
            # pdb.set_trace()
            self.model.saveTensorAsWeight()
        return {
    ...
    def _restore_best_state_model(self, train_type='pretrain', is_best=False):
        ...
        ###
        if is_best:
            state_dict_path = os.path.join(self.local_export_root, 'models', BEST_STATE_DICT_FILENAME + '_' + train_type + '_best')
        else:
