...
from meantime.models.transformer_models.GraphGAT_add_behavior import KGAT

from abc import *
from pathlib import Path
import os
import pdb
from meantime.dataloaders.graphGAT_add_behavior import GraphLoader
...
class GraphTrainer(AbstractTrainer):
    def __init__(self, args, model, train_loader, val_loader, test_loader, local_export_root):
        ...
        # pdb.set_trace()
        self.origin_m_item = self.graph_loader.origin_m_item
        self.origin_m_user = self.graph_loader.origin_m_user

        # pdb.set_trace()
        # self.graph_model = graph_model
        self.graph_model = KGAT(self.args, self.graph_loader).to(self.device)
    ...
    @classmethod
    def code(cls):
        return 'graph_sasrec_improve_kgat_add_behavoir'
    ...
    def trainGraphModelOneEpoch(self, optim_graph):
        ...
        for (batch_i,
            ...
            # loss, reg_loss = self.graph_model.tranR_loss(batch_users, batch_rels, batch_pos, batch_neg)
        ...
        optim_graph_kge = self.graph_opt_kge #同一个optim;
        ...
        for (batch_i,
            ...
            # tranR_loss, reg_loss = self.graph_model.bpr_loss(batch_users, batch_pos, batch_neg, batch_rels)
    ...
    def train(self):
        ...
        self.user_hidden_rep = self.user_hidden_rep[:self.origin_m_user, ]
        self.item_hidden_rep = self.item_hidden_rep[:self.origin_m_item, ]

        # self.model.setUserItemRepFromGraph(self.user_hidden_rep, self.item_hidden_rep) #每次加载相同的hidden representatin, 不合理;
        self.user_hidden_rep_cate_ori, self.item_hidden_rep_cate_ori = self.graph_model.getUserItemEmbOri()
        #setting representations to sequential models; 由于user embedding不参与模型, 两个输出的均是item表征;
        self.model.setUserItemRepFromGraph(None, None, self.user_hidden_rep, self.item_hidden_rep, None, None, self.user_hidden_rep_cate_ori, self.item_hidden_rep_cate_ori) #每次加载相同的hidden representatin, 不合理;
        #setting representations to sequential models; 由于user embedding不参与模型, 两个输出的均是item表征;
        # self.model.setUserItemRepFromGraph(self.user_hidden_rep, self.item_hidden_rep) #每次加载相同的hidden representatin, 不合理;
