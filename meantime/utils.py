...
def setup_train(args, machine_is_host=False):
    ...
    if machine_is_host:
        ...
        if os.path.exists(local_export_root):
            if exp_group == 'test' and args.finetune_flag == False: #当pretrain时, 删除之前保存的模型; finetune则加载之前的模型;
                print('Removing local test export root {}'.format(local_export_root))
                # shutil.rmtree(local_export_root) #预训练时, 不删除, 直接覆盖的方式实现;
            elif args.finetune_flag == True:
                print("For finetuning: {}".format(local_export_root))
            else:
    ...
    if args.resume_training:
        ...
        if os.path.exists(local_export_root):
            if args.experiment_group != 'test':
                ...
                # shutil.rmtree(local_export_root)
        if not os.path.exists(local_export_root):
            os.makedirs(local_export_root)

def export_config(args, local_export_root):
    sufflix = ''
    if args.finetune_flag == False:
        sufflix = 'pretrain'
    else:
        sufflix = 'finetune'
    with open(os.path.join(local_export_root, 'config_{}.json'.format(sufflix)), 'w') as outfile:
