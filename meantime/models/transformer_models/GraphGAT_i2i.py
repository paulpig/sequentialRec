...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
        ...
        self.W_R_value = nn.Parameter(torch.Tensor(self.attribute_nums, self.latent_dim, self.latent_dim))
        nn.init.normal_(self.W_R_value, std=0.1)
...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
    ...
    def getEmbeddingBothFromUsers(self, users, pos_users, neg_users):
        all_users, all_items = self.computer()
        users_emb = all_users[users]
    ...
    def i2i_loss(self, users, pos, neg, rel):
        # pdb.set_trace()
        #以user表征为中心, item分别正负样本;
        (users_emb, pos_emb, neg_emb, 
        userEmb0,  posEmb0, negEmb0) = self.getEmbeddingBothFromUsers(users.long(), pos.long(), neg.long())
        reg_loss = (1/2)*(userEmb0.norm(2).pow(2) + 
                         posEmb0.norm(2).pow(2)  +
                         negEmb0.norm(2).pow(2))/float(len(users))
        ...
        # pos_scores = torch.mul(users_emb, (pos_emb + rel_emb)) #(bs, 1)
        ...
        loss = torch.mean(torch.nn.functional.softplus(neg_scores - pos_scores))
        # loss = - torch.mean(torch.log(pos_scores + 1e-24) + torch.log(1.0 - neg_scores + 1e-24))
