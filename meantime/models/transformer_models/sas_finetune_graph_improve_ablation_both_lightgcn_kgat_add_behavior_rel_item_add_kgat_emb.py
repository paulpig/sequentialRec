...
class SASModel(BaseModel):
    ...
    @classmethod
    def code(cls):
        return 'graph_sasrec_improve_lightgcn_kgat_behavoir_rel_items_add_kgat_emb'
    ...
    def createMergeParameter(self):
        ...
        self.W1_para_6 = nn.Linear(self.args.latent_dim_rec, 1).cuda()
        ...
        self.W_graph_para_6_1 = nn.Linear(self.args.latent_dim_rec, self.args.latent_dim_rec).cuda()
    ...
    def get_logits(self, d):
        ...
        if self.args.merge_type == "gate":
            ...
            # pdb.set_trace()
            user_ori_emb_1 = self.user_ori[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1)
            user_ori_emb_2 = self.item_ori[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1)
            attr_emb_1 = self.k_user_ori[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1)

            seq_1 = self.W1_para_1(torch.tanh(self.W_graph_para_1_1(candidate_embeddings_graph_1)))#(bs, sl, 1)
            seq_2 = self.W1_para_2(torch.tanh(self.W_graph_para_2_1(candidate_embeddings_graph_2))) #(bs, sl, 1)
            seq_3 = self.W1_para_3(torch.tanh(self.W_graph_para_3_1(candidate_embeddings_graph_attribute))) #(bs, sl, 1)
            ori_lg_1 = self.W1_para_4(torch.tanh(self.W_graph_para_4_1(user_ori_emb_1))) #(bs, sl, 1)
            ori_lg_2 = self.W1_para_5(torch.tanh(self.W_graph_para_5_1(user_ori_emb_2))) #(bs, sl, 1)
            ori_kgat = self.W1_para_6(torch.tanh(self.W_graph_para_6_1(attr_emb_1))) #(bs, sl, 1)
            seq_merge = torch.cat([seq_1, seq_2, seq_3, ori_lg_1, ori_lg_2, ori_kgat], dim=-1)
            ...
            seq_merge_tensor = torch.cat([torch.unsqueeze(candidate_embeddings_graph_1, dim=-2), torch.unsqueeze(candidate_embeddings_graph_2, dim=-2), torch.unsqueeze(candidate_embeddings_graph_attribute, dim=-2), \
                torch.unsqueeze(user_ori_emb_1, dim=-2), torch.unsqueeze(user_ori_emb_2, dim=-2), torch.unsqueeze(attr_emb_1, dim=-2)], dim=-2) #(bs, sl, 5, dim)
            # pdb.set_trace()
            graph_e = (seq_merge_tensor * torch.unsqueeze(seq_merge_weight, dim=-1)).sum(-2) #(bs, sl, dim)
