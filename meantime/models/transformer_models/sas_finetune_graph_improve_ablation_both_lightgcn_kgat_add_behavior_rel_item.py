...
class SASModel(BaseModel):
    ...
    @classmethod
    def code(cls):
        return 'graph_sasrec_improve_lightgcn_kgat_behavoir_rel_items'
    ...
    def setUserItemRepFromGraph(self, user_rep_buy, item_rep_buy, user_rep_view, item_rep_view, user_ori, item_ori):
        """
    ...
    def get_logits(self, d):
        ...
        if self.args.merge_type == "gate":
            ...
            ori_embeddings_graph_2 = self.item_ori[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1)
            ...
            seq_merge_tensor = torch.cat([torch.unsqueeze(candidate_embeddings_graph_1, dim=-2), torch.unsqueeze(candidate_embeddings_graph_2, dim=-2), torch.unsqueeze(candidate_embeddings_graph_attribute, dim=-2), \
                torch.unsqueeze(ori_embeddings_graph_1, dim=-2), torch.unsqueeze(ori_embeddings_graph_2, dim=-2)], dim=-2) #(bs, sl, 5, dim)
            # pdb.set_trace()
            graph_e = (seq_merge_tensor * torch.unsqueeze(seq_merge_weight, dim=-1)).sum(-2) #(bs, sl, dim)
