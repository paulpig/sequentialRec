...
class LightGCNAttentionType(BertBaseModel):
    def __init__(self, config:dict, dataset):
        super(LightGCNAttentionType, self).__init__(config)
        self.config = config
...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
        ...
        self.relation_cate3 = self.embedding_rel_buy.weight

        self.embedding_rel_view = torch.nn.Embedding(
            num_embeddings=1, embedding_dim=self.latent_dim)
        nn.init.normal_(self.embedding_rel_view.weight, std=0.1)
        self.relation_price = self.embedding_rel_view.weight

        self.embedding_rel_brand = torch.nn.Embedding(
            num_embeddings=1, embedding_dim=self.latent_dim)
        nn.init.normal_(self.embedding_rel_brand.weight, std=0.1)
        self.relation_brand = self.embedding_rel_brand.weight
        # linear
    ...
    def initWeight(self):
        # all_weights = dict()
        self.gcn_linears_1 = nn.ModuleList([nn.Linear(self.latent_dim, self.latent_dim*2) for _ in range(self.n_layers)])
        self.gcn_linears_2 = nn.ModuleList([nn.Linear(self.latent_dim, self.latent_dim*2) for _ in range(self.n_layers)])
        self.gcn_linears_3 = nn.ModuleList([nn.Linear(self.latent_dim, self.latent_dim*2) for _ in range(self.n_layers)])
        ...
        self.rel_true_1 = nn.ModuleList([nn.Linear(self.latent_dim, self.latent_dim) for _ in range(self.n_layers)])
        self.rel_true_2 = nn.ModuleList([nn.Linear(self.latent_dim, self.latent_dim) for _ in range(self.n_layers)])
        self.rel_true_3 = nn.ModuleList([nn.Linear(self.latent_dim, self.latent_dim) for _ in range(self.n_layers)])
    ...
    def computer_inner(self, Graph_cate3, Graph_brand, Graph_price):
        ...
        # embs_brand = [all_emb]
        # embs_price = [all_emb]
        relation_cate3 = self.relation_cate3
        relation_price = self.relation_price
        relation_brand = self.relation_brand

        rels_cate3 = [relation_cate3]
        rels_price = [relation_price]
        rels_brand = [relation_brand]
        ...
        for layer in range(self.n_layers):
            ...
            # attention 方式整合
            relation_cate3 = self.rel_true_1[layer](relation_cate3) #(1, dim)
            relation_price = self.rel_true_2[layer](relation_price)
            relation_brand = self.rel_true_3[layer](relation_brand)
            seq_1 = self.rel_linears_1[layer](torch.tanh(self.gcn_linears_1[layer](torch.cat((all_emb_cate3, relation_cate3), dim=-1))))#(bs, sl, 1)
            seq_2 = self.rel_linears_2[layer](torch.tanh(self.gcn_linears_2[layer](torch.cat((all_emb_brand, relation_brand), dim=-1)))) #(bs, sl, 1)
            seq_3 = self.rel_linears_3[layer](torch.tanh(self.gcn_linears_3[layer](torch.cat((all_emb_price, relation_price), dim=-1)))) #(bs, sl, 1)
            ...
            rels_cate3.append(relation_cate3)
            rels_price.append(relation_price)
            rels_brand.append(relation_brand)

        embs = torch.stack(embs, dim=1)
        #print(embs.size())
        light_out = torch.mean(embs, dim=1)

        rels_cate3_embs = torch.stack(rels_cate3, dim=1)
        rels_cate3_embs = torch.mean(rels_cate3_embs, dim=1)

        rels_price_embs = torch.stack(rels_price, dim=1)
        rels_price_embs = torch.mean(rels_price_embs, dim=1)

        rels_brand_embs = torch.stack(rels_brand, dim=1)
        rels_brand_embs = torch.mean(rels_brand_embs, dim=1)

        users, items = torch.split(light_out, [self.num_users, self.num_items])
        return users, items, rels_cate3_embs, rels_price_embs, rels_brand_embs
