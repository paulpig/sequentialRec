...
class TransformerBlock(nn.Module):
    def __init__(self, args):
        ...
        self.attention = MultiHeadedAttention(h=attn_heads, d_model=hidden, dropout=dropout, args=args)
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, h, d_model, dropout=0.1, args=None):
        super().__init__()
        assert d_model % h == 0
        ...
        # if args.add_graph_flag: #输入维度变成2*d_model;
        #     self.linear_layers = nn.ModuleList([nn.Linear(d_model * 2, d_model) for _ in range(3)])
        # else:
