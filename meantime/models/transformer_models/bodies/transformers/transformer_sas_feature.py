...
class TransformerSasFeatureBlock(nn.Module):
    ...
    def forward(self, x, mask, abs_kernel, rel_kernel, rel_kernel_mask, layer, info):
        # x : B x T x H
        # abs_kernel : La of [B x T x H]
        # rel_kernel : Lr of [B x T x T x H]
        x = self.input_sublayer(x, lambda _x: self.attention(_x, _x, _x, mask, abs_kernel, rel_kernel, rel_kernel_mask, layer, info))
...
class MixedAttention(nn.Module):
    def __init__(self, args, La, Lr):
        ...
        self.La = La
        self.Lr = Lr
        self.d_k = d_model // h #分割成head numbers;
        self.h = h
        self.scale = 1 / (self.d_k ** 0.5)
        ## TODO
        self.content_linear_layers = nn.ModuleList([nn.Linear(d_model, d_model) for _ in range(3)])
        self.content_linear_layers_v2 = nn.ModuleList([nn.Linear(d_model, d_model) for _ in range(3)])
    ...
    def forward(self, query_input, key_input, value_input, mask, abs_kernel, rel_kernel, rel_kernel_mask, layer, info):
        # q, k, v : B x T x H
        # abs_kernel : La of [B x T x H]
        # rel_kernel : Lr of [B x T x T x H]
        # rel_kernel_mask: [bs, Lr, sl, sl]
        batch_size, T = query_input.size(0), query_input.size(1)

        # q, k, v, kernel_q, kernel_k : B x n x T x d
        query, key, value = \
            [l(x).view(batch_size, T, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.content_linear_layers, (query_input, key_input, value_input))]


        # scores = torch.matmul(query, key.transpose(-2, -1)) #(bs, n, sl, sl)
        ...
        if self.Lr > 0:
            ...
            # pdb.set_trace()
            # rel_scores += torch.einsum('blid,blijd->blij', Xq + self.rel_position_bias, R) * rel_kernel_mask.type(torch.FloatTensor).cuda()  # B x Lr x T x T
            rel_scores += torch.einsum('blid,blijd->blij', Xq, R) * rel_kernel_mask.type(torch.FloatTensor).cuda()  # B x Lr x T x T
            scores[:, self.La:] += rel_scores

        # query_v2, key_v2, value_v2 = \
        #     [l(x).view(batch_size, T, self.h, self.d_k).transpose(1, 2)
        #      for l, x in zip(self.content_linear_layers_v2, (query_input, key_input, value_input))]
        
        # if self.Lr >0:
        #     Xq = query_v2  # B x Lr x T x d  
        #     Xk = key  # B x Lr x T x d  
        #     R = torch.stack([l(x) for l, x in zip(self.rel_position_key_linear_layers, rel_kernel)], dim=1)  # B x Lr x T x T x d
        #     # rel_scores = torch.einsum('blid,blijd->blij', Xq + self.rel_position_bias, R) * rel_kernel_mask.type(torch.FloatTensor).cuda() #(bs, n, sl, sl)
        #     # pdb.set_trace()
        #     rel_scores = torch.einsum('blid,blijd->blij', Xq, R) * rel_kernel_mask.type(torch.FloatTensor).cuda() #(bs, n, sl, sl)
        #     # pdb.set_trace()
