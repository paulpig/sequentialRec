...
class SASModel(BaseModel):
    ...
    @classmethod
    def code(cls):
        return 'sas_finetune_graph_improve_merge_b_pretrain_based_user'
    ...
    def forward(self, d):
        ...
        if self.training:
            ...
            candidate_embeddings_graph = self.user_rep_graph[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1)
    ...
    def get_logits(self, d):
        ...
        graph_e = self.user_rep_graph[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1) #(bs, sl, dim) #use the hidden representation to init the embedding lookup table;
        #merge based gate
        gate = torch.sigmoid(self.W1_para(bert_pretrain_e) + self.W2_para(graph_e))
        merge_e = bert_pretrain_e * gate + graph_e * (1 - gate)
    ...
    def get_loss(self, logits, labels, negative_labels):
        ...
        valid_labels_emb_graph = self.user_rep_graph[valid_labels, :]  # M x H
        ...
        valid_negative_labels_emb_graph = self.user_rep_graph[valid_negative_labels, :]  # M x H
