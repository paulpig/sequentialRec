...
class SASModel(BaseModel):
    ...
    @classmethod
    def code(cls):
        return 'sas_finetune_graph_improve_based_user'
    ...
    def forward(self, d):
        ...
        if self.training:
            ...
            candidate_embeddings = self.user_rep_graph[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1) 
    ...
    def get_loss(self, logits, labels, negative_labels):
        ...
        valid_labels_emb = self.user_rep_graph[valid_labels, :]  # M x H
        valid_negative_labels_emb = self.user_rep_graph[valid_negative_labels, :]  # M x H
