...
class SASModel(BaseModel):
    ...
    @classmethod
    def code(cls):
        return 'graph_sasrec_improve_lightgcn_rm_kgat_with_init'
    ...
    def forward(self, d):
        ...
        if self.training:
            ...
            candidate_embeddings_graph_attribute = self.token_embedding.emb(d['candidates']).reshape(x.size(0), x.size(1), -1)
    ...
    def get_logits(self, d):
        ...
        # candidate_embeddings_graph_attribute = self.user_rep_graph_attribute[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1)
        candidate_embeddings_graph_attribute = self.token_embedding.emb(d['tokens']).reshape(x.size(0), x.size(1), -1)
    ...
    def get_loss(self, logits, labels, negative_labels):
        ...
        valid_labels_emb_graph_attribute = self.token_embedding.emb(valid_labels)
        ...
        valid_negative_labels_emb_graph_attribute = self.token_embedding.emb(valid_negative_labels)
