...
class SASModel(BaseModel):
    ...
    @classmethod
    def code(cls):
        return 'sas_finetune_graph_improve_merge_b_pretrain_double_graph_items'
    ...
    def forward(self, d):
        ...
        if self.training:
            ...
            candidate_embeddings_bert = self.token_embedding.emb(d['candidates'])  # B x C x H
            ...
            candidate_embeddings_graph = self.W_graph_para_1(candidate_embeddings_graph_1) + self.W_graph_para_2(candidate_embeddings_graph_2)
            gate = torch.sigmoid(self.W1_para(candidate_embeddings_bert) + self.W2_para(candidate_embeddings_graph))
            candidate_embeddings = candidate_embeddings_bert * gate + candidate_embeddings_graph * (1 - gate)
            # candidate_embeddings = candidate_embeddings_bert
    ...
    def get_logits(self, d):
        ...
        bert_pretrain_e = self.token_embedding(d)
        ...
        graph_e_2 = self.user_rep_graph[x_unsqueeenze, :].reshape(x.size(0), x.size(1), -1) #(bs, sl, dim) #use the hidden representation to init the embedding lookup table;
        #merge based gate
        graph_emb = self.W_graph_para_1(graph_e) + self.W_graph_para_2(graph_e_2)
        gate = torch.sigmoid(self.W1_para(bert_pretrain_e) + self.W2_para(graph_emb))
        merge_e = bert_pretrain_e * gate + graph_emb * (1 - gate)
        # e = graph_e + self.positional_embedding(d)

        e = merge_e + self.positional_embedding(d)
    ...
    def get_loss(self, logits, labels, negative_labels):
        ...
        valid_labels_emb_bert = self.token_embedding.emb(valid_labels)  # M x H
        #替换为graph model输出的item representation;
        ...
        valid_labels_emb_graph = self.W_graph_para_1(valid_labels_emb_graph_1) + self.W_graph_para_2(valid_labels_emb_graph_2)
        gate = torch.sigmoid(self.W1_para(valid_labels_emb_bert) + self.W2_para(valid_labels_emb_graph))
        valid_labels_emb = valid_labels_emb_bert * gate + valid_labels_emb_graph * (1 - gate)
        # valid_labels_emb = valid_labels_emb_bert

        valid_negative_labels_emb_bert = self.token_embedding.emb(valid_negative_labels)  # M x H
        ...
        valid_negative_labels_emb_graph = self.W_graph_para_1(valid_negative_labels_emb_graph_1) + self.W_graph_para_2(valid_negative_labels_emb_graph_2)
        gate = torch.sigmoid(self.W1_para(valid_negative_labels_emb_bert) + self.W2_para(valid_negative_labels_emb_graph))
        valid_negative_labels_emb = valid_negative_labels_emb_bert * gate + valid_negative_labels_emb_graph * (1 - gate)
        # valid_negative_labels_emb = valid_negative_labels_emb_bert
